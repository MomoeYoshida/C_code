...
/* #define DAY_FLAG for SLSTR Day-2 product (bit 8 of 16-bit l2p_flags, value = 128s) */
#define DAY_FLAG 128
...
static VINT readNetCDFData_short( int nc_id, const char *pName, 
				 size_t nx, size_t ny, signed short *pShort );
static VINT readNetCDFData_Byte_float( int nc_id, const char *pName, 
				       size_t nx, size_t ny, float *pVar );
static VINT get_Julian_Day( VINT mm, VINT id, VINT iyyy );
static VINT get_GHRSST_Time_from_Date( const VINT Year, const VINT Month, 
		  ...
		  VFLOAT output_stdev, VINT min_quality, VINT min_algorithm, VFLOAT min_wind_speed )
{
  ...
  signed short *pShort = NULL;
  signed short day_flag = DAY_FLAG;
  ...
  /* N.B. EUMETSAT decided to change the global attribute "start_time" to "time_coverage_start" for their Day-2 SLSTR products... */
  if(1 != day2){
    ...
    status = nc_get_att_text(nc_id,NC_GLOBAL,"time_coverage_start",start_time_string);
    if( NC_NOERR != status ){
      message(2,"ERROR: Cannot get time_coverage_start from GHRSST file ",pFilename);
      /* EXIT open fail */
      return(1);
    }
  }    
  ...
  /* ...and also the global attribute "stop_time" to "time_coverage_end" */
  
  if(1 != day2){
    status = nc_get_att_text(nc_id,NC_GLOBAL,"stop_time",stop_time_string);
    if( NC_NOERR != status ){
      message(2,"ERROR: Cannot get stop_time from GHRSST file ",pFilename);
      /* EXIT open fail */
      return(1);
    }
  } else {
    status = nc_get_att_text(nc_id,NC_GLOBAL,"time_coverage_end",stop_time_string);
    if( NC_NOERR != status ){
      message(2,"ERROR: Cannot get time_coverage_end from GHRSST file ",pFilename);
      /* EXIT open fail */
      return(1);
    }
  }    
  ...
  if( 0 == strncmp(pType,"ACSPO",5) ){
	...
	*(pByte+i) = 1;
  ...
  } else if( 0 == strncmp(pType,"SLSTR",5) ){
    ...
    if( (0 == day2) ){
      ...
      /* Now simply reuse algorithm information to flag day/night */
      for(i=0;i<nx*ny;i++){
...
/*  Day-2 processing can use separate dual-view product.  Requires some reworking because algorithm_type was used to separate day & night.  
 *  Also needs readNetCDFData_short(), which did not exist before (l2p_flag reader was specific to ACSPO flags...)
 */
     if( NULL == (pShort = (signed short *) calloc(nx*ny,sizeof(signed short))) ){
       message(1,"ERROR: Cannot allocate pShort (short) array");
       /* EXIT Short variable allocation fail */
       free(pVar);
       free(pByte);
       free(pByte2);
       exit(-1);
      ...
      if( 1 == readNetCDFData_short(nc_id,"l2p_flags",nx,ny,pShort) ){
        message(2,
            "ERROR: Error reading l2p_flags from SLSTR L2P file into short array",
            pFilename);
        status = nc_close(nc_id);
        /* Free what has already been allocated */
        free(pVar);
      ...
      /* Set bad values to bad based on min_quality - N.B. dual-view data is now a separate product */
      for(i=0;i<nx*ny;i++){
        if( (min_quality > *(pByte+i)) ){
	  *(pSST->array+i) = NaN;
        }
      ...
      /* Now simply reuse to flag day/night */
      for(i=0;i<nx*ny;i++){
        /* bit 8 (the highest of SLSTR Day-2 l2p_flags) is "day", i.e. 128.  N.B. This is very quick and dirty...
           ...** NO LONGER TRUE THAT IT'S THE HIGHEST VALUE **  Needs more precise testing of flags with bitwise operator... */
        if( (day_flag & *(pShort+i)) ){
        /* Daytime */
	  *(pByte+i) = 0;
        } else {
        /* Nighttime */
    ...
    /* Copy to output structure - may as well return for SLSTR as well as ACSPO - used in new SLSTR GHRSST reader (I imagine...) */
    copy_byte_to_str_matlab(nx,ny,(char *) pByte, pCldmask);
  ...
  /* Get wind speed & apply to daytime data */
	...
	*(pSST->array+i) = NaN;
    ...
    /* Free pByte2 here - it's only created if input is SLSTR */
    free(pByte2);
...
/*  if((NULL != pByte2)){
  ...
  if((NULL != pShort)){
    free(pShort);
  }  ** The if() test didn't work - still tried to free pByte2 after it had already been freed.  DON'T UNDERSTAND WHY ** */
...
/*  deltaTime = (stop_time-start_time)*1./pSST->ny; */
  deltaTime = (VFLOAT)(stop_time-start_time)/(VFLOAT)(pSST->ny);
  for(i=0;i<pTime->ny;i++){
    scanline_time = start_time + (VINT)(deltaTime*i);
    for(j=0;j<pTime->nx;j++){
 /*     *(pTime->array+j+i*pTime->nx) = (VINT) (start_time + deltaTime*i); */
      *(pTime->array+j+i*pTime->nx) = scanline_time;
    }
  }
...
{ 
  ...
  int one_bit_cld = 0;    /* ACSPO V2.9 (beta08) no longer has 2-bit cloud mask */
  ...
  if( NULL != pStr ){
    ...
    /*  ACSPO flag_meanings attribute has changed after v2.5, so check for 2 different strings  */
    if( 0 == strncmp(pStr,"probably_clear_or_cloudy_or_undefined",37) ){
      flag_cld_pos = 0;
    }
    /*  ACSPO flag_meanings attribute has changed again after v2.9, so check for 3 different strings  */
    if( 0 == strncmp(pStr,"SST_not_valid_or_degraded",25) ){
      flag_cld_pos = 0;
      one_bit_cld  = 1;
    }
    if( 0 == strncmp(pStr,"mask_undefined",14) ){
      flag_cld_pos = 0;
    }
    nflags = 1;
  } else {
  ...
  while( NULL != pStr ){
    ...
    /*  ACSPO flag_meanings attribute has changed after v2.5, so check for 2 different possibilities  */
      if( 0 == strncmp(pStr,"probably_clear_or_cloudy_or_undefined",37) ){
	flag_cld_pos = nflags;
      }
    /*  ACSPO flag_meanings attribute has changed again after v2.9, so check for 3 different possibilities  */
      if( 0 == strncmp(pStr,"SST_not_valid_or_degraded",25) ){
	flag_cld_pos = nflags;
      }
    /*  "mask_undefined" is name of token defined as -16384 (i.e. 1100000000000000b)  */
      if( 0 == strncmp(pStr,"mask_undefined",14) ){
	flag_cld_pos = nflags;
      }
      nflags++;
  ...
  /* Two bit mask */
  for(i=0;i<15;i++){    /*  Changed from i<31 to i<15 as it's a signed short int  */
    if( 1 == one_bit_cld ){
      value = (1<<i);
    } else {
      value = (3<<i);
  ...
  for(i=0;i<nx*ny;i++){
    ...
    *(pVar+i) = 0;
    ...
    /* Now cld mask - note this is two bits, except for most recent ACSPO V2.9 (beta08 - on) */
    cld_mask = ((*(pFlags+i)>>cloud_bit_pos) & 0x3);    /*  The position should be the same for pre-V2.9 and 2.9-on.  
                                                         *  The 2-bit mask is applied after the shift and will give the same result anyway  */
    *(pVar+i) = (*(pVar+i) | (cld_mask<<acspo_cld_mask));  /*  acspo_cld_mask is a (hardwired) 6-bit shift to the left  */
  }
...
{ 
  ...
  status = nc_get_vara_short( nc_id, varid, start, count, pShort );
  if( NC_NOERR != status ){
 ...
 *   Original version by Jonathan P.D. Mittaz on 11/04/2013
 *
 *   Modified to automatically read either signed or unsigned byte by Andy Harris on 07/19/2021
 *
 */
static VINT readNetCDFData_Byte_float( int nc_id, const char *pName, 
				       size_t nx, size_t ny, float *pVar )
{ 
  ...
  unsigned char *puByteVar = NULL;
  unsigned char *puByteVar_ptr = NULL;
  unsigned char ufill_value = 0;
  ...
  /* Get nc_type of variable.  Needed to discriminate between signed and unsigned byte */
  status = nc_inq_var( nc_id, varid, 0, &var_type, &var_ndims, var_dimids, &var_natts );
  if( NC_NOERR != status ){
    message(2,"ERROR: Cannot get nc_type for ",pName);
    return(1);
  }
  ...
  /* Test on var_type and read & process signed or unsigned byte accordingly */

  if( NC_UBYTE == var_type ){

    status = nc_get_att_ubyte( nc_id, varid, "_FillValue", &ufill_value );
    if( NC_NOERR != status ){
      message(2,"ERROR: Cannot get _FillValue for ",pName);
      return(1);
    }
    ...
    if( NULL == (puByteVar = (unsigned char *) calloc(nx*ny,sizeof(unsigned char))) ){
      message(1,"ERROR: Cannot allocate puByteVar for ",pName);
      /* EXIT cannot get variable id from file */
      exit(-1);
    }

    /* Read in 1 byte data */
    status = nc_get_vara_ubyte( nc_id, varid, start, count, puByteVar );
    if( NC_NOERR != status ){
      message(2,"ERROR: Cannot get float variable for ",pName);
      return(1);
    }
    ...
    /* Scale to floating point */
    puByteVar_ptr = puByteVar;
    for(i=0;i<nx*ny;i++){
      ubyteval = *(puByteVar_ptr++);
      if( ufill_value == ubyteval ){
        *(pVar+i) = NaN;
      } else {
        *(pVar+i) = ubyteval*scale + offset;
      }
    }
