 ...
 * NAME: 
 *
 *    process_raw_slstr_mex
 *
 * FUNCTION:
 *
 *    C interface to MATLAB for process_raw_slstr_ghrsst_c.c - reads in Sentinel-3 SLSTR data
 *
 * TYPE/LANGUAGE: 
 *
 *    C module
 ...
 * DESCRIPTION:  
 *
 *    Get input arrays from MATLAB and run process_raw_slstr_mex which
 *    reads in day+night or day or night data from the Sentinel-3 SLSTR datasets
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  char slstr_name[MAX_STRING_LENGTH];
  ...
  struct float_struct BiasNight; /* ** No Bias needed for SLSTR as it is the reference, but it will be set to zero ** */
...
++idebug;
  ...
  if( 10 != nrhs ){
    mexErrMsgIdAndTxt("MEX:process_raw_slstr_mex",
...
++idebug;
#endif
  /* check that slstr_name is a string */
  if( 0 == mxIsClass(prhs[0],"char") ){
    mexErrMsgIdAndTxt("MEX:process_raw_slstr_mex",
		      "First argument (slstr_name) must be character string");
  }
...
++idebug;
#endif
  /* From input MATLAB arguments get slstr_name, year, day, ref_sst,
   * sst_variability and sst_check */
  if( 1 == mxGetString( prhs[0], slstr_name, MAX_STRING_LENGTH ) ){
    message(1,"ERROR: Getting SLSTR name");
    exit(-1);
  }
...
#endif
  /* Call C code */
  ok = process_raw_slstr_ghrsst_c( slstr_name, year, day, &Ref_SST, 
