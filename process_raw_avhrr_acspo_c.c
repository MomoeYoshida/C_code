 ...
 *
 */
VINT process_raw_avhrr_acspo_c( const char *noaa_name, const VINT year, 
				const VINT day, 
...
/*  unsigned char start_day_night = 0;
  ...
  if( NULL != strstr(nameStr,"NOAA16") ){
    /* NOAA-16 */
    noaa_no = NOAA_16;
  } else if( NULL != strstr(nameStr,"NOAA17") ){
    /* NOAA-17 */
    noaa_no = NOAA_17;
  } else if( NULL != strstr(nameStr,"NOAA18") ){
    /* NOAA-18 */
    noaa_no = NOAA_18;
  } else if( NULL != strstr(nameStr,"NOAA19") ){
    /* NOAA-19 */
    noaa_no = NOAA_19;
  } else if( NULL != strstr(nameStr,"METOPA") ){
    /* Metop A */
    noaa_no = METOPA;
  } else if( NULL != strstr(nameStr,"METOPB") ){
    /* Metop B */
    noaa_no = METOPB;
  } else if( NULL != strstr(nameStr,"METOPC") ){
    /* Metop C */
    noaa_no = METOPC;
  } else if( NULL != strstr(nameStr,"VIIRS") ){
    /* S-NPP */
    noaa_no = VIIRS;
  } else if( NULL != strstr(nameStr,"JPSS") ){
    /* Metop B */
    noaa_no = JPSS;
  } else {
    message(2,"ERROR: Unknown AVHRR type ",nameStr);
    exit(-1);
  }
  ...
  case NOAA_16:
    ...
    if( 0 == strlen(file_info.dir_noaa_16_nav) ){
      message(1,"ERROR: dir_noaa_16_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_noaa_16_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_noaa_16_sst,MAX_STRING_LENGTH);
    ...
    std_default_day = par_info.noaa16_day_c0_std_default;
    std_default_night = par_info.noaa16_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.noaa16_sses_bias;
    sses_stdev = par_info.noaa16_sses_stdev;
    use_sses_stdev = par_info.noaa16_use_sses_stdev;
    strcpy(acspo_format,par_info.noaa16_acspo_format);
    break;
  case NOAA_17:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_noaa_17_nav) ){
      message(1,"ERROR: dir_noaa_17_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_noaa_17_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_noaa_17_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
			    par_info.noaa17_day_c0_threshold_mult);
    add_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.noaa17_day_c0_std_default;
    std_default_night = par_info.noaa17_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.noaa17_sses_bias;
    sses_stdev = par_info.noaa17_sses_stdev;
    use_sses_stdev = par_info.noaa17_use_sses_stdev;
    strcpy(acspo_format,par_info.noaa17_acspo_format);
    break;
  case NOAA_18:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_noaa_18_nav) ){
      message(1,"ERROR: dir_noaa_18_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_noaa_18_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_noaa_18_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.noaa18_day_c0_std_default;
    std_default_night = par_info.noaa18_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.noaa18_sses_bias;
    sses_stdev = par_info.noaa18_sses_stdev;
    use_sses_stdev = par_info.noaa18_use_sses_stdev;
    strcpy(acspo_format,par_info.noaa18_acspo_format);
    break;
  case NOAA_19:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_noaa_19_nav) ){
      message(1,"ERROR: dir_noaa_19_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_noaa_19_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_noaa_19_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.noaa19_day_c0_std_default;
    std_default_night = par_info.noaa19_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.noaa19_sses_bias;
    sses_stdev = par_info.noaa19_sses_stdev;
    use_sses_stdev = par_info.noaa19_use_sses_stdev;
    strcpy(acspo_format,par_info.noaa19_acspo_format);
    break;
  case METOPA:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_metopa_nav) ){
      message(1,"ERROR: dir_metopa_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_metopa_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_metopa_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.METOPA_day_c0_std_default;
    std_default_night = par_info.METOPA_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.metopa_sses_bias;
    sses_stdev = par_info.metopa_sses_stdev;
    use_sses_stdev = par_info.metopa_use_sses_stdev;
    strcpy(acspo_format,par_info.metopa_acspo_format);
    break;
  case METOPB:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_metopb_nav) ){
      message(1,"ERROR: dir_metopb_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_metopb_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_metopb_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.METOPB_day_c0_std_default;
    std_default_night = par_info.METOPB_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.metopb_sses_bias;
    sses_stdev = par_info.metopb_sses_stdev;
    use_sses_stdev = par_info.metopb_use_sses_stdev;
    strcpy(acspo_format,par_info.metopb_acspo_format);
    break;
  case METOPC:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_metopc_nav) ){
      message(1,"ERROR: dir_metopc_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_metopc_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_metopc_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.METOPC_day_c0_std_default;
    std_default_night = par_info.METOPC_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.metopc_sses_bias;
    sses_stdev = par_info.metopc_sses_stdev;
    use_sses_stdev = par_info.metopc_use_sses_stdev;
    strcpy(acspo_format,par_info.metopc_acspo_format);
    break;
  case VIIRS:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_viirs_nav) ){
      message(1,"ERROR: dir_viirs_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_viirs_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_viirs_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.VIIRS_day_c0_std_default;
    std_default_night = par_info.VIIRS_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.viirs_sses_bias;
    sses_stdev = par_info.viirs_sses_stdev;
  ...
  case JPSS:
    ...
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_jpss_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_jpss_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.JPSS_day_c0_std_default;
    std_default_night = par_info.JPSS_night_c0_std_default;
    /* GHRSST */
    correct_bias = par_info.jpss_sses_bias;
    sses_stdev = par_info.jpss_sses_stdev;
    use_sses_stdev = par_info.jpss_use_sses_stdev;
    strcpy(acspo_format,par_info.jpss_acspo_format);
    break;
  default:
    sprintf(messageStr,
	    "*** NOAA type is invalid. Choose from noaa16, 17, 18, 19, METOPA/METOPB/METOPC, VIIRS or JPSS.'");
  ...
  if( NULL != strstr(acspo_format,"HDF") ){
    use_GHRSST = 0;
    use_NETCDF = 0;
    use_HDF = 1;
  } else {
    if( NULL != strstr(acspo_format,"NETCDF") ){
      use_GHRSST = 0;
      use_NETCDF = 1;
      use_HDF = 0;
    } else {
  ...
  if( 1 == use_HDF ){
    strncpy(daystr,get_avhrr_datestring(year,day),MAX_STRING_LENGTH);
    strncpy(datestring,get_avhrr_datestring2(year,day),MAX_STRING_LENGTH);
  } else if( 1 == use_GHRSST ){
    /* For GHRSST format */
    strncpy(datestring,get_ghrsst_datestring(year,day),MAX_STRING_LENGTH);
  } else {
    /* For NetCDF ACSPO format */
    strncpy(datestring,get_avhrr_datestring2(year,day),MAX_STRING_LENGTH);
  }
  ...
  if( (compare_sst_Night.nx != Threshold_Night.nx) ||
    ...
    message(1,"ERROR: Climatology/Tresholds have difference sizes (process_raw_avhrr_acspo_old)");
    /* EXIT */
    exit(-1);
  }

  /* Get list of ACSPO files */
  /* Get correct string depending on in NOAA- or METOP or VIIRS or JPSS */
  if( 1 == use_HDF ){
    switch( noaa_no ){
    case NOAA_16:  
      sprintf(searchStr,"%s/ACSPO_*_GHRR*_%s_*.hdf",directory_sst,datestring);
      break;
    case NOAA_17:  
      sprintf(searchStr,"%s/ACSPO_*_GHRR*_%s_*.hdf",directory_sst,datestring);
      break;
    ...
    case NOAA_19:  
      ...
      break;
    case METOPA:  
      sprintf(searchStr,"%s/ACSPO_*_METOPA_*_%s_*.hdf",directory_sst,datestring);
      break;
    case METOPB:  
      sprintf(searchStr,"%s/ACSPO_*_METOPB_*_%s_*.hdf",directory_sst,datestring);
      break;
    case METOPC:  
      sprintf(searchStr,"%s/ACSPO_*_METOPC_*_%s_*.hdf",directory_sst,datestring);
      break;
    case VIIRS:  
      sprintf(searchStr,"%s/ACSPO_*_VIIRS_*_%s_*.hdf",directory_sst,
	      datestring);
      break;
    case JPSS:  
      sprintf(searchStr,"%s/ACSPO_*_N20_*_%s_*.hdf",directory_sst,
	      datestring);
      break;
    }
  } else if( 1 == use_GHRSST ){
    switch( noaa_no ){
    case NOAA_16:  
      sprintf(searchStr,"%s/%s*-L2P_GHRSST-*.nc",directory_sst,datestring);
      break;
    case NOAA_17:  
      sprintf(searchStr,"%s/%s*-L2P_GHRSST-*.nc",directory_sst,datestring);
      break;
    ...
    switch( noaa_no ){
    case NOAA_16:  
      sprintf(searchStr,"%s/ACSPO-NOAA16*_s%s*.nc",directory_sst,
	      datestring);
      break;
    case NOAA_17:  
      sprintf(searchStr,"%s/ACSPO-NOAA17*_s%s*.nc",directory_sst,
	      datestring);
      break;
    case NOAA_18:  
      sprintf(searchStr,"%s/ACSPO-NOAA18*_s%s*.nc",directory_sst,
	      datestring);
      break;
    case NOAA_19:  
      sprintf(searchStr,"%s/ACSPO-NOAA19*_s%s*.nc",directory_sst,
	      datestring);
      break;
    case METOPA:  
      sprintf(searchStr,"%s/ACSPO_*_METOPA*_%s*.nc",directory_sst,
	      datestring);
      break;
    case METOPB:  
      sprintf(searchStr,"%s/ACSPO_*_METOPB*_%s*.nc",directory_sst,
	      datestring);
      break;
    case METOPC:  
      sprintf(searchStr,"%s/ACSPO_*_METOPC*_%s*.nc",directory_sst,
	      datestring);
      break;
    case VIIRS:  
      sprintf(searchStr,"%s/ACSPO_NPP_VIIRS*_%s*.nc",directory_sst,
	      datestring);
      break;
    case JPSS:  
      sprintf(searchStr,"%s/ACSPO_NPP_N20*_%s*.nc",directory_sst,
	      datestring);
      break;
    }
  }
  ...
  if( 0 == find_files(searchStr,&nFiles,&ppFilelist) ){
    ...
    if( NOAA_16 <= noaa_no ){
      sprintf(messageStr,"*** No files found for NOAA-%d for %4.4d day %3.3d.",
	      noaa_no,year,day);
    } else {
      sprintf(messageStr,"*** No files found for METOPA/METOPB/METOPC/VIIRS/JPSS for %4.4d day %3.3d.",
	      year,day);
    }
    message(1,messageStr);
    message(1,
					      ...
					      VINT *pNBadStd, VINT use_sses_stdev )
{
  ...
  if( 0 < max_stored ){
    ...
    allocate_float_1d_array_matlab(MAX_STORE,&Sorted);
    allocate_float_1d_array_matlab(MAX_STORE,&Sorted_Var);
...
/*    if( finite(*(pSST->array+i)) && *(pSST->array+i) > sst_min && ((mask == mask_1) || (mask == mask_2)) ){  */
