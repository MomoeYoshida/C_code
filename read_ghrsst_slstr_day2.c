 ...
 *
 */
VINT read_ghrsst_slstr_day2( const char *pFilename, struct int_struct *pTime,
...
/*  Previous version used WST product that contained a mix of dual-view & nadir SSTs  */
 ...
 *  Also needed readNetCDFData_short(), which did not exist before (l2p_flag reader was specific to ACSPO flags...)
 */
	...
	*(pSST->array+i) = NaN;
    ...
    /* Now simply reuse to flag day/night */
    for(i=0;i<nx*ny;i++){
      /* bit 8 (the highest of SLSTR Day-2 l2p_flags) is "day", i.e. 256.  N.B. This is very quick and dirty...*/
      if( (256 <= *(pShort+i)) ){
      /* Daytime */
	*(pByte+i) = 0;
      } else {
	...
	*(pByte+i) = 1;
    ...
    message(2,"ERROR: Cannot recognize read_ghrsst_slstr_day2 type : ",pType);
    /* EXIT error in type */
    exit(-1);
  }
