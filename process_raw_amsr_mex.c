 ...
 * NAME: 
 *
 *    process_raw_amsr_mex
 *
 * FUNCTION:
 *
 *    C interface to MATLAB for process_raw_amsr_c.c - reads in AVHRR data
 *
 * TYPE/LANGUAGE: 
 *
 *    C module
 ...
 *          process_raw_avhrr_acspo_mex(amsr_name,year,day,...
 *                                      ref_sst,...
 *                                      sst_variability,...
 *                                      sst_check,bias_day,...
 ...
 * INPUTS
 *
 *    amsr_name         - make of AVHRR type to read
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  char amsr_name[MAX_STRING_LENGTH];
  ...
  if( 10 != nrhs ){
    mexErrMsgIdAndTxt("MEX:process_raw_amsr_mex",
  ...
  /* check that amsr_name is a string */
  if( 0 == mxIsClass(prhs[0],"char") ){
    mexErrMsgIdAndTxt("MEX:process_raw_amsr_mex",
		      "First argument (amsr_name) must be char");
  }
  ...
  /* From input MATLAB arguments get amsr_name, year, day, ref_sst,
   * sst_variability and sst_check */
  if( 1 == mxGetString( prhs[0], amsr_name, MAX_STRING_LENGTH ) ){
    message(1,"ERROR: Getting AMSR name");
    exit(-1);
  }
  ...
  if( 1 == mxGetString(prhs[8],logfile_stream,MAX_STRING_LENGTH) ){
    message(1,"ERROR: Getting type string");
    exit(-1);
  }
  ...
  /* If not a 2-d array then set to zero */
  if( 1 == bias_day ){
  ...
  /* Call C code */
  /* GHRSST-specific AMSR processing defined here */

  if( 0 == strncmp(par_info.amsr_format,"GHRSST",6) ){
    ok = process_raw_amsr_ghrsst_c( amsr_name, year, day, &Ref_SST, 
			    &SST_Variability, sst_check, &BiasDay,
			    &BiasNight, &OutData[0], par_files );
  } else {
    /* Can only process AMSR data in GHRSST format */
    message(1,"ERROR: Unrecognized AMSR format (not GHRSST)");
    ok=1;
  }
