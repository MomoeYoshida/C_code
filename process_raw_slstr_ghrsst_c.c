 ...
 * FUNCTION:
 *
 *    C version of process_raw_slstr.m - reads in SLSTR data
 *
 * TYPE/LANGUAGE: 
 *
 *    C module
 ...
 * DESCRIPTION:  
 *
 *    Contains the routine to read in Sentinel-3 SLSTR data.  Note that this works
...
#include "read_slstr_ghrsst.h"
#include "process_raw_slstr_ghrsst_c.h"
 ...
 * FUNCTION:
 *
 *    C Code to read in Sentinel-3 SLSTR data for one day
 *
 * TYPE/LANGUAGE: 
 *
 *    C routine
				...
				const struct par_files_str par_files )
{
  ...
  /* Data from Sentinel-3 SLSTR GHRSST L2P files */
  ...
  char use_GHRSST = 0;	/*  Only GHRSST L2P format  */
  ...
  strcpy(nameStr,slstr_name);
  for(i=0;i<strlen(nameStr);i++){    
    nameStr[i] = toupper(nameStr[i]);
  }
  if( NULL != strstr(nameStr,"SLSTRA2") ){
    /* Sentinel-3A */
    slstr_no = S3A2;
  } else if( NULL != strstr(nameStr,"SLSTRB2") ){
    /* Sentinel-3B */
    slstr_no = S3B2;
  } else if( NULL != strstr(nameStr,"SLSTRA") ){
    /* Sentinel-3A */
    slstr_no = S3A;
  } else if( NULL != strstr(nameStr,"SLSTRB") ){
    /* Sentinel-3B */
    slstr_no = S3B;
  } else {
    message(2,"ERROR: Unknown SLSTR type ",nameStr);
    exit(-1);
  }
  ...
  case S3A2:
    ...
    if( 0 == strlen(file_info.dir_slstra_nav) ){
      message(1,"ERROR: dir_slstra_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_slstra_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_slstra_sst,MAX_STRING_LENGTH);
    ...
    std_default_day = par_info.slstra_day_std_default;
    std_default_night = par_info.slstra_night_std_default;
    /* GHRSST */
    correct_bias = par_info.slstra_sses_bias;
    sses_stdev = par_info.slstra_sses_stdev;
    use_sses_stdev = par_info.slstra_use_sses_stdev;
    strcpy(slstr_format,par_info.slstr_format);
    break;
  case S3B2:
    /* Check to see if there is an entry */
    if( 0 == strlen(file_info.dir_slstrb_nav) ){
      message(1,"ERROR: dir_slstrb_nav/sst is not defined");
      /* EXIT */
      exit(-1);
    }
    /* Get directory names */
    strncpy(directory_nav,file_info.dir_slstrb_nav,MAX_STRING_LENGTH);
    strncpy(directory_sst,file_info.dir_slstrb_sst,MAX_STRING_LENGTH);
    /* Day threshold for quality control */
    mult_float_array_matlab(&Threshold_Day,
    ...
    std_default_day = par_info.slstrb_day_std_default;
    std_default_night = par_info.slstrb_night_std_default;
    /* GHRSST */
    correct_bias = par_info.slstrb_sses_bias;
    sses_stdev = par_info.slstrb_sses_stdev;
    use_sses_stdev = par_info.slstrb_use_sses_stdev;
    strcpy(slstr_format,par_info.slstr_format);
    break;
  case S3A:
    /* Check to see if there is an entry */
  ...
  default:
    sprintf(messageStr,
	    "*** SLSTR type is invalid. Choose from SLSTRA or SLSTRB.'");
...
/*      use_NETCDF = 0;
  ...
  /* Get datestring - get_datestring2 specific for SLSTR */
    /* For GHRSST format */
  ...
  /* Get list of SLSTR files.  Only GHRSST L2P format to consider... */
    switch( slstr_no ){
    case S3A2:  
      sprintf(searchStr,"%s/%s*-L2P_GHRSST-SSTskin-SLSTRA_NRT_DUAL*.nc",directory_sst,
	      datestring);
      break;
    case S3B2:  
      sprintf(searchStr,"%s/%s*-L2P_GHRSST-SSTskin-SLSTRB_NRT_DUAL*.nc",directory_sst,
	      datestring);
      break;
    case S3A:  
      sprintf(searchStr,"%s/%s*-L2P_GHRSST-SSTskin-SLSTRA-*.nc",directory_sst,
	      datestring);
      break;
    case S3B:  
      sprintf(searchStr,"%s/%s*-L2P_GHRSST-SSTskin-SLSTRB-*.nc",directory_sst,
	      datestring);
      break;
    }
  ...
  if( 0 == find_files(searchStr,&nFiles,&ppFilelist) ){
    message(3,"*** SLSTR directory is ",directory_sst,".");
    /* Output instrument type specific error message */
    if( (S3A == slstr_no) || (S3A2 == slstr_no) ){
      sprintf(messageStr,"*** No files found for Sentinel-3A SLSTR for %4.4d day %3.3d.",
	      year,day);
    } else {
      sprintf(messageStr,"*** No files found for Sentinel-3B SLSTR for %4.4d day %3.3d.",
	      year,day);
    }
    message(1,messageStr);
    message(1,
  ...
  if( 1 == par_info.correct_diurnal_warming ){
    ...
    if( 1 == read_diurnal_file(DWfile) ){
      ...
      message(1,"ERROR: Turning off DW correction (SLSTR processing)");
  ...
  for(i=0;i<nFiles;i++){
    /* Read in SLSTR data */
    /* Note, if requested, diurnal warming is corrected for within read_slstr_ghrsst */
    if( 0 == read_slstr_ghrsst( *(ppFilelist+i), slstr_format, &SST, &Lat, &Lon, 
	...
	/* Make our own cloud mask first - SLSTR GHRSST data are already screened vs. QL, algorithm type, & windspeed (for daytime) */
